...
{
    template<typename> class SIMDJsonObj;

    struct SIMDJsonConverter
    {
        //using JsonObj = simdjson::ParsedJson;
        using JsonVal = simdjson::simdjson_result<simdjson::fallback::ondemand::value>;

        static const auto getField(simdjson::ondemand::document& doc, const char* fieldName)
        {
            return doc[fieldName];
        }

        //De-serialize 
        template<typename A, typename B, typename std::enable_if<std::is_same_v<A,B>>::type* = nullptr>
        static A toType(const B& data)
        {
            return data;
        }

        template<class T, typename std::enable_if<std::is_base_of<Cerializer::SIMDJsonObj<T>, T >::value>::type* = nullptr>
        ...
        //Serialize
        template <class T>
        static void fromType(simdjson::ondemand::document& doc, Cerializer::SIMDJsonObj<T>& val)
        {
            //val.appendJsonStr(writer);
        }

        template<class T>
        static void fromType(simdjson::ondemand::document& doc, const Cerializer::SIMDJsonObj<T>& val)
        {
            //val.appendJsonStr(writer);
        }
        ...
        template <class T>
        static void fromType(simdjson::ondemand::document& doc, const unsigned short val)
        {
            //writer.Uint(val);
        }
        ...
        template <class T>
        static void fromType(simdjson::ondemand::document& doc, const unsigned int val)
        {
            //writer.Uint(val);
        }
        ...
        template <class T>
        static void fromType(simdjson::ondemand::document& doc, const unsigned long val)
        {
            //writer.Uint(val);
        }
        ...
        template <class T>
        static void fromType(simdjson::ondemand::document& doc, const std::wstring& val)
        {
            //writer.String(stringConverter.to_bytes(val).c_str());
        }

        template <class T>
        static void fromType(simdjson::ondemand::document& doc, const std::string& val)
        {
            //writer.String(val.c_str());
        }

        template <class T>
        static void fromType(simdjson::ondemand::document& doc, const std::shared_ptr<typename T::element_type>& ptr)
        {
            if (ptr)
            {
        ...
        template <class T>
        static void fromType(simdjson::ondemand::document& doc, typename T::element_type* ptr)
        {
            if (ptr)
            {
                fromType<T::element_type>(doc, ptr);
        ...
        template<class T>
        static void convertToJsonArray(simdjson::ondemand::document& doc, const T& container)
        {
        ...
        template<class T>
        static void fromType(simdjson::ondemand::document& doc, const std::vector<typename T::value_type>& inputVector)
        {
            convertToJsonArray(doc, inputVector);
        }

        template<class T>
        static void fromType(simdjson::ondemand::document& doc, const std::set<typename T::value_type>& inputSet)
        {
