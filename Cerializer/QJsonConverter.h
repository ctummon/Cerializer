...
{
    template<typename> class QJsonObj;
    struct QJsonConverter
    {
        ...
        template<class T, typename std::enable_if<std::is_base_of<Cerializer::QJsonObj<T>, T >::value>::type* = nullptr>
        ...
        template<class T, typename std::enable_if<std::is_same<QString, T>::value>::type* = nullptr>
        static T toType(const QJsonValue& data)
        {
            QString returnVal;
            if (data.isString())
        ...
        template <class T>
        static QJsonValue fromType(const std::shared_ptr<typename T::element_type>& ptr)
        {
            if (ptr)
            {
        ...
        template<class T>
        static QJsonValue fromType(const std::vector<typename T::value_type>& inputVector)
        {
