...
struct NlohmannJsonConverter
{
    ...
    static void populateJsonObj(const char* fieldName,
      JsonObj& data,
      const JsonObj::value_type& memberVar)
    {
    ...
    template<class T,
      typename std::enable_if<std::is_base_of<Cerializer::NlohmannJsonObj<T>,
    ...
    static T toType(const JsonObj::value_type& data)
    {
        ...
        if (data.is_string()) {
            returnVal =
              Utils::stringConverter.from_bytes(data.get<std::string>());
        }
        return returnVal;
    }
    ...
    template<class T>
    static JsonObj::value_type fromType(Cerializer::NlohmannJsonObj<T>& val)
    {
        return val.toJson();
    }

    template<class T>
    static JsonObj::value_type fromType(
      const Cerializer::NlohmannJsonObj<T>& val)
    {
        return val.toJson();
    }
    ...
    {
        return JsonObj::value_type(Utils::stringConverter.to_bytes(val));
    }
